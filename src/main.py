...
from handlers.uma_handler import UMA_Handler

LogHandler.get_instance().load_config("PEP", "./config/log_config.yaml")
...
# PDP Policy Handler
pdp_policy_handler = policy_handler(pdp_url=g_config["pdp_url"], pdp_port=g_config["pdp_port"],
...
def generate_rsa_key_pair():
    ...
    generated_private_key = _rsakey.exportKey()
    file_out = open("config/private.pem", "wb+")
    file_out.write(generated_private_key)
    file_out.close()

    generated_public_key = _rsakey.publickey().exportKey()
    file_out = open("config/public.pem", "wb+")
    file_out.write(generated_public_key)
    file_out.close()
...
resources_app.secret_key = ''.join(choice(ascii_lowercase) for j in range(30))  # Random key
...
# Mode load
if is_partial_mode_enabled():
    ext_interface_app.register_blueprint(authorize.construct_blueprint(oidc_client, uma_handler, g_config))
else:
    ext_interface_app.register_blueprint(
        proxy.construct_blueprint(oidc_client, uma_handler, g_wkh, g_config, private_key))
            ...
            try:
                ...
                if "T&C" in k:
                    payload = {"resource_scopes": k["scopes"], "icon_uri": k["resource_uri"], "name": k["name"],
                               "description": k["description"], "T&C": k["T&C"]}
                else:
                    payload = {"resource_scopes": k["scopes"], "icon_uri": k["resource_uri"], "name": k["name"],
                               "description": k["description"]}
                res = post(
                    "http://" + g_config["service_host"] + ":" + str(g_config["resources_service_port"]) + "/resources",
                    headers=headers, json=payload, verify=False)
                id_res = res.text
                logger.info("==========New Resource for URI: \"" + k[
                    "resource_uri"] + "\" with ID: \"" + id_res + "\"==========")
            except Exception as e:
                logger.info(
                    "==========Default resources operation threw an exception for resource " + k["name"] + "==========")
                logger.info(str(e))
